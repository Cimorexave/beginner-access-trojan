...
`bad-script` file makes a connection request from the computer to the listener server and connects through a reverse shell where the listener server running the `server-script` file is listening for a connection and once receiving it; connects to the device's powershell; hence giving the server users access to the host's shell.
